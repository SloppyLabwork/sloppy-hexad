...
function NameAndDecksForm(props) {
  ...
  const handleSubmit = (event) => {
    ...
    if (!playerName) {
      ...
      window.alert('Use six different decks, if you please')
    } else {
      ...
      <div className="game-status">
        <div className="alert alert-info">
          <div className="fs-4 mb-2">
            {IS_JOIN ? (
              <span>
                You've been invited to a Hexad match. Use the form below to
                join.
              </span>
            ) : (
              <span>Use the form below to start a Hexad match.</span>
            )}
          </div>
        </div>
      </div>
